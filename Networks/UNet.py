...
class UNet(nn.Module):
    def __init__(self, in_nc=1, out_nc=1, nc=[64, 128, 256, 512], nb=2, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose'):
        ...
        self.m_up3 = B.sequential(upsample_block(nc[3]*2, nc[2], mode='2'+act_mode), *[B.conv(nc[2], nc[2], mode='C'+act_mode) for _ in range(nb)])
        self.m_up2 = B.sequential(upsample_block(nc[2]*2, nc[1], mode='2'+act_mode), *[B.conv(nc[1], nc[1], mode='C'+act_mode) for _ in range(nb)])
        self.m_up1 = B.sequential(upsample_block(nc[1]*2, nc[0], mode='2'+act_mode), *[B.conv(nc[0], nc[0], mode='C'+act_mode) for _ in range(nb)])

        self.m_tail = B.conv(nc[0]*2, out_nc, bias=True, mode='C')
    ...
    def forward(self, x0):
        ...
        x4 = self.m_down3(x3)       
        x = self.m_body(x4)
        x = self.m_up3(torch.cat([x,x4],dim=1))
        x = self.m_up2(torch.cat([x,x3],dim=1))
...
class UNetRes(nn.Module):
    ...
    def forward(self, x0):
        sigma = 1/255
        std_map = torch.tensor(sigma, device=x0.device).reshape([1] * x0.ndim)
        std_shape_exp = [x0.shape[0], 1, x0.shape[2], x0.shape[3]]
        x0 = torch.cat((x0, std_map.expand(std_shape_exp)), dim=1)
...
class UNetResLight(nn.Module):
    def __init__(self, in_nc=1, out_nc=1, nc=[64, 128], nb=2, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose'):
        super(UNetResLight, self).__init__()
        ...
        self.m_down1 = B.sequential(*[B.ResBlock(nc[0], nc[0], bias=False, mode='C'+act_mode+'C') for _ in range(nb)], downsample_block(nc[0], nc[1], bias=False, mode='2'))
        self.m_body  = B.sequential(*[B.ResBlock(nc[1], nc[1], bias=False, mode='C'+act_mode+'C') for _ in range(nb)])
...
class NonExpansiveUNetRes(nn.Module):
    def __init__(self, in_nc=1, out_nc=1, nc=[64, 128, 256, 512], nb=4, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose'):
        super(NonExpansiveUNetRes, self).__init__()

        self.m_head = B.conv(in_nc, nc[0], bias=False, mode='Y')
        ...
        self.m_down1 = B.sequential(*[B.ResBlock(nc[0], nc[0], bias=False, mode='Y'+act_mode+'Y') for _ in range(nb)], downsample_block(nc[0], nc[1], bias=False, mode='2'))
        self.m_down2 = B.sequential(*[B.ResBlock(nc[1], nc[1], bias=False, mode='Y'+act_mode+'Y') for _ in range(nb)], downsample_block(nc[1], nc[2], bias=False, mode='2'))
        self.m_down3 = B.sequential(*[B.ResBlock(nc[2], nc[2], bias=False, mode='Y'+act_mode+'Y') for _ in range(nb)], downsample_block(nc[2], nc[3], bias=False, mode='2'))

        self.m_body  = B.sequential(*[B.ResBlock(nc[3], nc[3], bias=False, mode='Y'+act_mode+'Y') for _ in range(nb)])
        ...
        self.m_up3 = B.sequential(upsample_block(nc[3], nc[2], bias=False, mode='2'), *[B.ResBlock(nc[2], nc[2], bias=False, mode='Y'+act_mode+'Y') for _ in range(nb)])
        self.m_up2 = B.sequential(upsample_block(nc[2], nc[1], bias=False, mode='2'), *[B.ResBlock(nc[1], nc[1], bias=False, mode='Y'+act_mode+'Y') for _ in range(nb)])
        self.m_up1 = B.sequential(upsample_block(nc[1], nc[0], bias=False, mode='2'), *[B.ResBlock(nc[0], nc[0], bias=False, mode='Y'+act_mode+'Y') for _ in range(nb)])

        self.m_tail = B.conv(nc[0], out_nc, bias=False, mode='Y')
