...
import numpy as np
import torch
from Networks.Utils import Utils_Image as util
...
def test_mode(model, L, mode=0, refield=32, min_size=256, sf=1, modulo=1,iteration=0):
    '''
    # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    # Some testing modes
    # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    '''
    x8_mode = iteration%8
    L = augment_img_tensor4(L, mode=x8_mode)
    if mode == 0:
        E = test(model, L)
    elif mode == 1:
    ...
    E = augment_img_tensor4(E, mode=x8_invert_mode)
    return E
